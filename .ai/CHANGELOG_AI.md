...
## 2026-07-30 — Run lifecycle webhook delivery

- **Task:** Fire outbound webhooks when runs complete/fail/cancel
- **Source revision before change:** bb0b2a3
- **Source revision after change:** TBD
- **Files modified:** internal/api/handlers_runs.go, internal/agent/agent.go
- **Summary:**
  - Add WebhookURL field to TestRun (optional outbound webhook endpoint)
  - Fire webhook on run completion with state, metrics, error details
  - Best-effort async delivery (goroutine) — webhook failures don't block run lifecycle
  - Payload includes: run_id, state, passed/failed/total counts, error message
  - Works for all terminal states: StateDone, StateFailed, StateCancelled
- **Reason:** Enable CI/CD integration — trigger downstream jobs when tests complete
- **Risk:** Low — async goroutine, webhook failure logged but not propagated
- **Breaking changes:** None
- **Database migrations:** None (WebhookURL stored as JSON field on TestRun)
- **Deployment steps:** None
- **Documentation updated:** CHANGELOG_AI.md
- **Verification completed:** go test ./... -short — all 25 packages pass
- **Facts added/removed or confidence changed:** None
- **Open unknowns:** None
- **Related ADRs/TODOs:** None

## 2026-07-30 — LLM resilience with exponential backoff and circuit breaker

- **Task:** Make LLM calls resilient to transient failures
- **Source revision before change:** 8ddb6a4
- **Source revision after change:** bb0b2a3
- **Files modified:** internal/ai/client.go, internal/ai/resilient.go (new), internal/ai/resilient_test.go (new)
- **Summary:**
  - Implement ResilientClient decorator with retry + circuit breaker
  - Exponential backoff: 1s → 2s → 4s (capped at 8s), with jitter
  - Circuit breaker: opens after 5 failures, half-open after 30s, closes on success
  - Retryable errors: connection refused/reset, timeout, HTTP 429/5xx
  - Non-retryable: 4xx client errors, context cancelled, malformed requests
  - Configurable timing via SetResilienceTimings/ResetResilienceTimings for tests
  - TestMain sets fast timings (1ms backoff) so test suite runs in <2s instead of 57s
- **Reason:** LLM APIs have transient failures (rate limits, timeouts) — retry improves reliability
- **Risk:** Low — decorator pattern, transparent to callers, configurable
- **Breaking changes:** None
- **Database migrations:** None
- **Deployment steps:** None
- **Documentation updated:** CHANGELOG_AI.md
...
## 2026-07-31 — Fix tree-sitter parsers: deps, compile errors, extraction bugs
...
- **Summary:**
  ...
  - Fixed ParseCtx argument order (ctx, oldTree, content) in all parsers
  - Replaced Child/ChildCount with NamedChild/NamedChildCount when iterating call arguments (anonymous tokens like '(' broke extraction; JS/Go/Python routes returned 0)
  - PHP: Route::get is a scoped_call_expression; fixed node types, $table/$fillable extraction via property_element/property_initializer, middleware string+array handling; fixed scopeName typo
  - Python: Django route/view extraction fixed; views now require a request/self first parameter; raw-string prefix stripped before quote trim
  - Added exported DetectFramework(string) (string, error) to PHP and Python parsers to satisfy the parser.Parser interface
  - Deleted stub duplicate internal/parser/golang (real implementation is internal/parser/go, package golang)
  - Added parser.NewDefaultRegistry() registering javascript/go/php/python parsers; NewRegistry() stays empty for tests
- **Reason:** Phase 1 parsers were documented as complete but did not compile; extraction logic was untested against the real tree-sitter grammars
- **Risk:** Low — changes confined to internal/parser; full test suite passes
- **Breaking changes:** None (internal/parser/golang removed; it was an unused stub)
- **Database migrations:** None
- **Deployment steps:** None
- **Documentation updated:** CHANGELOG_AI.md
- **Verification completed:** go build ./..., go vet ./internal/parser/..., go test ./... — all packages pass
- **Facts added/removed or confidence changed:** FINAL_SUMMARY.md claim "all parsers implemented and tested" was inaccurate before this change (build was broken)
- **Open unknowns:** internal/github test-generation service still not wired into the server
- **Related ADRs/TODOs:** None

## 2026-07-31 — Wire GitHub test-generation service into the API server
...
- **Summary:**
  ...
  - webhook.PushEvent now parses commits[] with added/removed/modified files
  - New Server.processPushWithTestGen: on push, when AI planning is enabled (GOTEST_AI_PLANNING=1), clones the repo, parses it with parser.NewDefaultRegistry(), generates a test plan via ai.SynthesisService, and launches the run; otherwise falls back to the previous plain auto-triggered run
  - ProcessPushEvent now skips non-branch refs and trims refs/heads/ before checkout (was checking out the raw ref)
  - Added Integration.repoPath guarding against webhook-supplied repository names escaping the clone base directory (path traversal)
- **Reason:** The richer webhook stack (clone, parse, AI test plan) existed but was dead code; the wired endpoint only created a generic run
- **Risk:** Low — default behavior unchanged unless GOTEST_AI_PLANNING is enabled; fallback preserved on any pipeline failure
- **Breaking changes:** NewTestGenerationService signature changed (agent → launch func); internal-only, no external callers
- **Database migrations:** None
- **Deployment steps:** Optional: set GOTEST_AI_PLANNING=1 and GITHUB_WEBHOOK_SECRET to enable webhook-driven test generation
- **Documentation updated:** CHANGELOG_AI.md
- **Verification completed:** go build ./..., go vet, go test ./... — all packages pass; new unit test covers disabled-AI fallback
- **Facts added/removed or confidence changed:** internal/github webhook stack is now wired (previously dead code)
- **Open unknowns:** Drift detection service and webhook_registrations/drifts tables still not implemented
- **Related ADRs/TODOs:** None

## 2026-07-31 — Add drift detection service (Phase 3)

- **Task:** Detect drift between source code and tests on GitHub push events (docs/PHASE-3-IMPLEMENTATION.md Sprint 15 scope)
- **Source revision before change:** 7cdb61b
- **Source revision after change:** 680789f
- **Files modified:** internal/drift/store.go (new), internal/drift/detector.go (new), internal/drift/detector_test.go (new), internal/db/migrations/010_drifts.sql (new), internal/api/handlers_drifts.go (new), internal/api/handlers_drifts_test.go (new), internal/api/server.go
- **Summary:**
  - New internal/drift package: Drift type (missing_test/outdated_test/removed_test; severity high/medium/low; status pending/fixed/ignored), in-memory Store (RWMutex + counter IDs, modeled on internal/notify), and Detector
  - Detector.DetectDrift compares push-changed files against per-language test conventions (Go _test.go, JS/TS .test/.spec/__tests__, Python test_*/*_test, PHP *Test.php): source changed without its test → missing_test (high) or outdated_test (medium, if test exists on disk in the clone dir); test removed while source remains → removed_test (high); source removed leaving orphaned test → removed_test (low)
  - Disk checks are path-traversal guarded (webhook-supplied file names cannot escape the repo dir)
  - New API endpoints: GET /api/v1/drifts (?repository=&type=&status=) and PATCH /api/v1/drifts/{id} ({"status": "fixed"|"ignored"|"pending"})
  - Webhook push callback now triggers drift detection asynchronously before test generation
  - Migration 010_drifts.sql adds webhook_registrations, drifts, drift_generated_tests tables per the Phase 3 schema (in-memory store used for now; DB persistence is a follow-up)
- **Reason:** Phase 3 continuous-sync milestone; drift detection was fully unimplemented
- **Risk:** Low — additive; detection runs in a goroutine and only writes to an in-memory store
- **Breaking changes:** None
- **Database migrations:** 010_drifts.sql (additive, CREATE TABLE IF NOT EXISTS)
- **Deployment steps:** None required; migration applies automatically where migrations run
- **Documentation updated:** CHANGELOG_AI.md
- **Verification completed:** go build ./..., go test -short ./... — all packages pass; 8 drift unit tests + 2 API tests added
- **Facts added/removed or confidence changed:** TestE2E_ProductSmoke failure observed is environmental (remote LLM provider 500 "database or disk is full"), not caused by this change
- **Open unknowns:** Drift store is in-memory only (lost on restart); auto-generation of tests from drifts (drift_generated_tests) and webhook registration service not yet implemented
- **Related ADRs/TODOs:** None

## 2026-07-31 — Review fixes for drift detection webhook integration
...
- **Summary:**
  ...
  - Added dedup: DetectDrift now skips creating a drift when an identical pending drift (repository+type+file_path) already exists; fixed/ignored drifts are re-detected on later pushes
  - Removed unused Store.Get; replaced with Store.HasPending used by the dedup
- **Reason:** Issues found during self-review of milestone 3
- **Risk:** Low — behavior fixes within the new feature
- **Breaking changes:** None
- **Database migrations:** None
- **Deployment steps:** None
- **Documentation updated:** CHANGELOG_AI.md
- **Verification completed:** go vet + go test -short on drift/api/webhook packages — all pass; new dedup test added
- **Facts added/removed or confidence changed:** Drift API endpoints confirmed behind apiKeyAuth and 1 MiB body limit; webhook HMAC verification confirmed before callback
- **Open unknowns:** Dedup is in-memory only; restarting the server clears drift history
- **Related ADRs/TODOs:** None

## 2026-07-31 — Add drift test auto-generation service (Phase 3)

- **Task:** Auto-generate tests for detected drifts via the LLM and expose review endpoints (Phase 3 "Auto-Generation Service", drift_generated_tests)
- **Source revision before change:** f468421
- **Source revision after change:** 75e6b40
- **Files modified:** internal/drift/generator.go (new), internal/drift/generator_test.go (new), internal/drift/store.go, internal/api/handlers_drift_tests.go (new), internal/api/handlers_drift_tests_test.go (new), internal/api/server.go
- **Summary:**
  - New drift.Generator: builds a language-aware prompt from a Drift, calls ai.Client.GenerateText, extracts the fenced code block and a trailing "Confidence: N" score (clamped 0-100), and stores the result in a GeneratedTestStore
  - GeneratedTest type + in-memory GeneratedTestStore (status generated/reviewed/rejected; ByDrift newest-first; UpdateStatus validation)
  - removed_test drifts are rejected for generation (source no longer exists → 422)
  - Restored Store.Get (drift lookup by ID) for the generate endpoint
  - New API endpoints (all behind apiKeyAuth): POST /api/v1/drifts/{id}/generate-test, GET /api/v1/drifts/{id}/generated-tests, PATCH /api/v1/generated-tests/{id}
  - generate-test returns 503 when AI planning is disabled (GOTEST_AI_PLANNING), 404 for unknown drift, 201 with the generated test on success
- **Reason:** Phase 3 milestone 4 (auto-generation); completes the detect → generate → review loop
- **Risk:** Low — additive; generation only runs on explicit POST and requires AI planning enabled
- **Breaking changes:** None
- **Database migrations:** None (drift_generated_tests table already added in 010_drifts.sql; store is in-memory for now)
- **Deployment steps:** Set GOTEST_AI_PLANNING=1 + LLM credentials to enable the generate endpoint
- **Documentation updated:** CHANGELOG_AI.md
- **Verification completed:** go vet + go test -short ./... — all 32 packages pass; 6 generator unit tests + 3 API tests added
- **Facts added/removed or confidence changed:** None
- **Open unknowns:** Generated tests are in-memory only and are not yet written to the repo or attached to a run; no automatic generation on drift detection (manual trigger only)
- **Related ADRs/TODOs:** None
