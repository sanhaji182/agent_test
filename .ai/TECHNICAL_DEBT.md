...
| DL-2 | `internal/agent/llm_*.go` vs `internal/ai/client.go` | Two LLM client layers (execution vs planning). Routing inconsistency fixed 2026-07-28: both factories accept the same normalized provider set; hosted providers resolve per-provider default base URLs via shared `ai.DefaultOpenAICompatibleBaseURL`; parity guarded by `TestProviderRoutingParity`. Remaining: transport-code merge — design proposed in **ADR-006** (4-step migration, `ai` becomes the single transport, `agent.LLM` interface unchanged) | MEDIUM (was HIGH) |
